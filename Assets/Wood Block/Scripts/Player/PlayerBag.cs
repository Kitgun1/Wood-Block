...
namespace WoodBlock
{
	public static class PlayerBag
	{
		...
		{
			MaxScore = Cloud.GetValue(nameof(MaxScore), MaxScore);
			CoinAmount = Cloud.GetValue(nameof(CoinAmount), CoinAmount);
			CancelMoveAmount = Cloud.GetValue(nameof(CancelMoveAmount), CancelMoveAmount);
			ClearTableAmount = Cloud.GetValue(nameof(ClearTableAmount), ClearTableAmount);
		}

		public static void Save()
		{
			Cloud.SetValue(nameof(MaxScore), MaxScore);
			Cloud.SetValue(nameof(CoinAmount), CoinAmount);
			Cloud.SetValue(nameof(CancelMoveAmount), CancelMoveAmount);
			Cloud.SetValue(nameof(ClearTableAmount), ClearTableAmount);
		}
	}
}